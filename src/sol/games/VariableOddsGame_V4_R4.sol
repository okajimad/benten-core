...
contract VariableOddsGame_V4_R4    	is GameBase_V4_R4, IVariableOddsGame {
    string private OutputFile = "games\VariableOddsGame_V4_R4.sol";
    struct Entry {
        uint32 odds;
        bytes4 content;
        uint volume; 
    ...
    function oddsList() public view returns(bytes4[], uint[]) {
        bytes4[] memory candidates = new bytes4[](_currentOdds.length);
    ...
    function updateOdds(bytes4 content, uint32 value) public onlyOwner {
        if(getNow() >= _lockTime) 
            {setLastError("betting not acceptable"); return;}
        uint32 index = findOrCreateOdds(content);
        _currentOdds[index].odds = value;
    }
    function updateOdds_Multi(bytes4[] contents, uint32[] values) public onlyOwner {
        if(getNow() >= _lockTime) 
            {setLastError("betting not acceptable"); return;}
    ...
    function findOrCreateOdds(bytes4 content) internal returns(uint32) {
    ...
    function findOddsIndex(bytes4 content) internal view returns(int32) {
        for(uint i = 0; i<_currentOdds.length; i++) {
            if(content == _currentOdds[i].content)
                return int32(i);
        }
    ...
    function estimateTotalRefund(bytes4 truth) public view returns(uint) {
        uint total_refund = 0;
    ...
    function calculateRefundWithOdds(bytes4 content, bytes4 truth, uint odds, uint volume) public pure returns(uint) {
        if(content==truth)
            return permilMul(volume, odds); //odds is permil, TODO overflow check
        else
            return 0;
    }
    function calculateRefundWithOdds_Wide(bytes4 content, bytes8 truth, uint odds, uint volume) public pure returns(uint) {
