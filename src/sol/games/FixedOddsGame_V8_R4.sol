...
*/
contract FixedOddsGame_V8_R4    	is GameBase_V8_R4{
    string private OutputFile = "games\FixedOddsGame_V8_R4.sol";
    struct Entry {
        bytes8 content;
        uint volume;
    ...
    function currentBettingList() public view returns(bytes8[], uint[], uint[] ) {
    ...
    function findOrCreate(bytes8 content) internal returns(uint32) {
        for(uint i = 0; i<_bettings.length; i++) {
    ...
    function findEntryIndex(bytes8 content) internal view returns(uint32) {
        for(uint i = 0; i<_bettings.length; i++) {
            if(content == _bettings[i].content)
                return uint32(i);
        }
